...
genere 2026-08-26 19:33 UTC — 60984 systemes evalues
...
- moteur actif `0caa9267db60` (17242 systemes) : `06fe04a859f1`, `0a74109317e5`, `23303c299f39`, `62448a55927e`, `84fba70921e0`, `8f22f0d2d442`, `b71bb0907fb5`, `e40600351a72`, `e80bc1b2b437`, `e8a1f930f7b9`
...
| connect | 4 | 3 | 27735 | 13833 | 1891 | 933 | 2741 | 2201 | 2946 | 2841 | 10.2% |
...
- candidats AVEC connectivite : 2968, dont T2 : 100%
- candidats SANS connectivite : 4484, dont T2 : 100%
- **l'hypothese ne tient pas — le v2 n'est qu'un v1 elargi**

### censure de l'echantillon

- **3220 systemes avec CONNECTED sur 25196 (12.8%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
*3162 candidats sur 7452 portent les champs bruts (42%).*
...
- `e3baecf8377b` — 657 candidats
  ...
  - test de permutation : **p = 0.0005** — significatif.
- `62448a55927e` — 293 candidats
  - AVEC connectivite (124) : resistance **39.7%**
...
- `e3baecf8377b` — 657 candidats
  ...
  - test de permutation : **p = 0.0005** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `62448a55927e` — 293 candidats
  - AVEC connectivite (124) : T0=13.06 T1=0.00 T2=2.93 — pondere **5.85**
  - SANS connectivite (169) : T0=16.09 T1=0.00 T2=2.55 — pondere **5.10**
  - test de permutation : **p = 0.0420** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `23303c299f39` — 225 candidats
...
**6 serie(s) reproductible(s) sur 12 etablissent l'ecart** : `89c65c03c4ad` (p=0.0005), `06fe04a859f1` (p=0.0005), `e3baecf8377b` (p=0.0005), `62448a55927e` (p=0.0420), `23303c299f39` (p=0.0060), `b71bb0907fb5` (p=0.0110).
...
- TROP-CHER : 3241 systemes abandonnes (5.3% des systemes), 96% du temps total
  dont 3220 avec CONNECTED, 21 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 13.0% sur 22695 systemes
  - sans CONNECTED : 0.0% sur 5040 systemes
