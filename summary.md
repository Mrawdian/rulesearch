...
genere 2026-08-26 13:05 UTC — 45642 systemes evalues
...
- moteur actif `0caa9267db60` (1900 systemes) : `06fe04a859f1`, `e40600351a72`
...
**7466 enregistrements (16%) proviennent d'un moteur dont la source n'existe plus** — ni dans git, ni sur le disque. Donnee valide mais non rejouable : ne pas la citer comme reproductible.
...
| connect | 4 | 3 | 19979 | 9984 | 1303 | 687 | 2005 | 1577 | 2143 | 2033 | 10.2% |
| d4 | 4 | 4 | 5991 | 2749 | 1523 | 30 | 421 | 404 | 284 | 430 | 7.2% |
| ref | 4 | 3 | 19548 | 4588 | 8262 | 13 | 2690 | 1008 | 7 | 2980 | 15.2% |
...
- candidats AVEC connectivite : 2160, dont T2 : 100%
- candidats SANS connectivite : 3290, dont T2 : 100%
- **l'hypothese ne tient pas — le v2 n'est qu'un v1 elargi**

### censure de l'echantillon

- **2417 systemes avec CONNECTED sur 18808 (12.9%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
*1160 candidats sur 5450 portent les champs bruts (21%).*
...
- `e40600351a72` — 194 candidats
  ...
  - test de permutation : **p = 0.0005** — significatif.
- `06fe04a859f1` — 45 candidats
  - AVEC connectivite (22) : resistance **37.5%**
  - SANS connectivite (23) : resistance **20.8%**
  - test de permutation : **p = 0.0035** — significatif.
...
- `06fe04a859f1` — 45 candidats
  - AVEC connectivite (22) : T0=13.64 T1=0.00 T2=2.45 — pondere **4.91**
  - SANS connectivite (23) : T0=16.74 T1=0.00 T2=2.65 — pondere **5.30**
  - test de permutation : **p = 0.6612** — **NON SIGNIFICATIF**, l'ecart est compatible avec le bruit. Ne pas conclure.
...
**2 serie(s) reproductible(s) sur 6 etablissent l'ecart** : `89c65c03c4ad` (p=0.0005), `e3baecf8377b` (p=0.0005).
...
- `T2` indices=0.12 — PAIRDIFF(>=2)@adj + PAIRDIFF(>=2)@knight + CONNECTED(v0) + COUNT(v0,1-4)@grid
- `T2` indices=0.12 — PAIRDIFF(>=1)@adj + CONNECTED(v2) + COUNT(v2,1-3)@grid
...
## cout
- temps total 14.2 h, dont 2% brule sur des systemes MORT
- TROP-CHER : 2434 systemes abandonnes (5.3% des systemes), 95% du temps total
  dont 2417 avec CONNECTED, 17 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 13.1% sur 16307 systemes
  - sans CONNECTED : 0.0% sur 3672 systemes
