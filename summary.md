...
genere 2026-08-26 01:53 UTC — 17022 systemes evalues

## versions du DSL presentes
- `89c65c03c4ad` : 8048 systemes
...
**7466 enregistrements (44%) proviennent d'un moteur dont la source n'existe plus** — ni dans git, ni sur le disque. Donnee valide mais non rejouable : ne pas la citer comme reproductible.
...
| connect | 4 | 3 | 8550 | 4304 | 557 | 287 | 814 | 685 | 915 | 884 | 10.3% |
| ref | 4 | 3 | 8348 | 2017 | 3499 | 2 | 1121 | 409 | 3 | 1297 | 15.5% |
...
- candidats AVEC connectivite : 887, dont T2 : 100%
- candidats SANS connectivite : 1301, dont T2 : 100%
...
- **915 systemes avec CONNECTED sur 7080 (12.9%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
- `89c65c03c4ad` — 1023 candidats
  - AVEC connectivite (408) : T0=13.39 T1=0.00 T2=2.86 — pondere **5.71**
  - SANS connectivite (615) : T0=15.91 T1=0.00 T2=2.67 — pondere **5.34**
  - test de permutation : **p = 0.0600** — **NON SIGNIFICATIF**, l'ecart est compatible avec le bruit. Ne pas conclure.
...
- `T2` indices=0.12 — CONNECTED(v0) + NOSQUARE(v0) + COUNT(v0,4-7)@grid + PAIRSTEP(1)@adj + PAIRDIFF(>=1)@knight
- `T2` indices=0.12 — PAIRDIFF(>=1)@knight + PAIRDIFF(>=1)@adj + CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,1-3)@grid
...
- `T2` indices=0.14 — CONNECTED(v0) + COUNT(v0,3-5)@grid + PAIRSTEP(1)@adj + PAIRDIFF(>=1)@knight
- `T2` indices=0.14 — PAIRDIFF(>=1)@knight + CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,3-5)@grid + PAIRSTEP(1)@adj
- `T2` indices=0.14 — CONNECTED(v0) + NOSQUARE(v0) + COUNT(v0,4-8)@grid + PAIRDIFF(>=1)@knight + PAIRSTEP(1)@adj

## cout
- temps total 5.3 h, dont 2% brule sur des systemes MORT
- TROP-CHER : 918 systemes abandonnes (5.4% des systemes), 96% du temps total
  dont 915 avec CONNECTED, 3 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 13.0% sur 7023 systemes
  - sans CONNECTED : 0.0% sur 1527 systemes
