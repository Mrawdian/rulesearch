...
genere 2026-08-27 02:40 UTC — 76566 systemes evalues

## versions du DSL presentes
- `89c65c03c4ad` : 29619 systemes
- `ab89a63b01ef` : 11659 systemes
...
- moteur actif `0caa9267db60` (32824 systemes) : `06fe04a859f1`, `0a74109317e5`, `23303c299f39`, `62448a55927e`, `84fba70921e0`, `8f22f0d2d442`, `ab89a63b01ef`, `b71bb0907fb5`, `cf6a4d3246d8`, `e40600351a72`, `e80bc1b2b437`, `e8a1f930f7b9`
...
**7526 enregistrements (10%) proviennent d'un moteur dont la source n'existe plus** — ni dans git, ni sur le disque. Donnee valide mais non rejouable : ne pas la citer comme reproductible.
...
| connect | 4 | 3 | 35603 | 17714 | 2448 | 1208 | 3529 | 2820 | 3788 | 3637 | 10.2% |
| d4 | 4 | 4 | 5991 | 2749 | 1523 | 30 | 421 | 404 | 284 | 430 | 7.2% |
| ref | 4 | 3 | 34848 | 8030 | 14824 | 20 | 4822 | 1819 | 19 | 5314 | 15.2% |
...
- candidats AVEC connectivite : 3764, dont T2 : 100%
- candidats SANS connectivite : 5624, dont T2 : 100%
- **l'hypothese ne tient pas — le v2 n'est qu'un v1 elargi**

### censure de l'echantillon

- **4062 systemes avec CONNECTED sur 31642 (12.8%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
*5098 candidats sur 9388 portent les champs bruts (54%).*
...
- `ab89a63b01ef` — 1421 candidats
  - AVEC connectivite (584) : resistance **41.2%**
  - SANS connectivite (837) : resistance **22.1%**
  - test de permutation : **p = 0.0005** — significatif.
...
- `ab89a63b01ef` — 1421 candidats
  - AVEC connectivite (584) : T0=12.97 T1=0.00 T2=3.04 — pondere **6.09**
  - SANS connectivite (837) : T0=15.93 T1=0.00 T2=2.70 — pondere **5.39**
  - test de permutation : **p = 0.0005** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `06fe04a859f1` — 1195 candidats
...
**6 serie(s) reproductible(s) sur 13 etablissent l'ecart** : `89c65c03c4ad` (p=0.0005), `ab89a63b01ef` (p=0.0005), `06fe04a859f1` (p=0.0005), `e3baecf8377b` (p=0.0005), `23303c299f39` (p=0.0060), `b71bb0907fb5` (p=0.0110).
...
- `T2` indices=0.12 — PAIRSTEP(1)@knight + CONNECTED(v0) + NOSQUARE(v0) + COUNT(v0,1-4)@grid + PAIRDIFF(>=1)@knight

## cout
- temps total 23.8 h, dont 2% brule sur des systemes MORT
- TROP-CHER : 4091 systemes abandonnes (5.3% des systemes), 96% du temps total
  dont 4062 avec CONNECTED, 29 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 13.0% sur 29141 systemes
  - sans CONNECTED : 0.0% sur 6462 systemes
