...
genere 2026-08-27 19:43 UTC — 111676 systemes evalues

## versions du DSL presentes
- `89c65c03c4ad` : 29619 systemes
- `5c556c16ea8b` : 28332 systemes
...
- moteur actif `0caa9267db60` (67934 systemes) : `06fe04a859f1`, `0a74109317e5`, `23303c299f39`, `5c556c16ea8b`, `62448a55927e`, `84fba70921e0`, `8f22f0d2d442`, `9e54e42681ed`, `ab89a63b01ef`, `b71bb0907fb5`, `cf6a4d3246d8`, `e40600351a72`, `e80bc1b2b437`, `e8a1f930f7b9`
...
**7585 enregistrements (7%) proviennent d'un moteur dont la source n'existe plus** — ni dans git, ni sur le disque. Donnee valide mais non rejouable : ne pas la citer comme reproductible.
...
| connect | 4 | 3 | 53317 | 26547 | 3696 | 1819 | 5277 | 4214 | 5625 | 5445 | 10.2% |
| d4 | 4 | 4 | 5991 | 2749 | 1523 | 30 | 421 | 404 | 284 | 430 | 7.2% |
| ref | 4 | 3 | 52244 | 12079 | 22183 | 26 | 7222 | 2717 | 25 | 7990 | 15.3% |
...
- candidats AVEC connectivite : 5572, dont T2 : 100%
- candidats SANS connectivite : 8300, dont T2 : 100%
- **l'hypothese ne tient pas — le v2 n'est qu'un v1 elargi**

### censure de l'echantillon

- **5899 systemes avec CONNECTED sur 46062 (12.8%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
*9582 candidats sur 13872 portent les champs bruts (69%).*
...
- `5c556c16ea8b` — 3617 candidats
  - AVEC connectivite (1444) : resistance **42.2%**
  - SANS connectivite (2173) : resistance **22.5%**
  - test de permutation : **p = 0.0005** — significatif.
...
- `5c556c16ea8b` — 3617 candidats
  - AVEC connectivite (1444) : T0=12.94 T1=0.00 T2=3.07 — pondere **6.14**
  - SANS connectivite (2173) : T0=15.77 T1=0.00 T2=2.71 — pondere **5.42**
  - test de permutation : **p = 0.0005** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `ab89a63b01ef` — 2280 candidats
...
**7 serie(s) reproductible(s) sur 14 etablissent l'ecart** : `89c65c03c4ad` (p=0.0005), `5c556c16ea8b` (p=0.0005), `ab89a63b01ef` (p=0.0005), `06fe04a859f1` (p=0.0005), `e3baecf8377b` (p=0.0005), `23303c299f39` (p=0.0060), `b71bb0907fb5` (p=0.0110).
...
- `T2` indices=0.12 — PAIRDIFF(>=1)@adj + PAIRDIFF(>=1)@knight + CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,1-5)@grid
...
- `T2` indices=0.12 — PAIRDIFF(>=1)@knight + PAIRDIFF(>=1)@adj + CONNECTED(v2) + COUNT(v2,1-3)@grid
- `T2` indices=0.12 — PAIRDIFF(>=1)@knight + CONNECTED(v1) + NOSQUARE(v1) + COUNT(v1,1-5)@grid + PAIRDIFF(>=1)@adj

## cout
- temps total 34.4 h, dont 2% brule sur des systemes MORT
- TROP-CHER : 5934 systemes abandonnes (5.3% des systemes), 96% du temps total
  dont 5899 avec CONNECTED, 35 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 12.9% sur 43561 systemes
  - sans CONNECTED : 0.0% sur 9756 systemes
