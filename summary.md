...
genere 2026-08-27 10:26 UTC — 93433 systemes evalues

## versions du DSL presentes
- `89c65c03c4ad` : 29619 systemes
- `ab89a63b01ef` : 18378 systemes
- `5c556c16ea8b` : 10089 systemes
...
- moteur actif `0caa9267db60` (49691 systemes) : `06fe04a859f1`, `0a74109317e5`, `23303c299f39`, `5c556c16ea8b`, `62448a55927e`, `84fba70921e0`, `8f22f0d2d442`, `9e54e42681ed`, `ab89a63b01ef`, `b71bb0907fb5`, `cf6a4d3246d8`, `e40600351a72`, `e80bc1b2b437`, `e8a1f930f7b9`
...
**7585 enregistrements (8%) proviennent d'un moteur dont la source n'existe plus** — ni dans git, ni sur le disque. Donnee valide mais non rejouable : ne pas la citer comme reproductible.
...
| connect | 4 | 3 | 44111 | 21985 | 3027 | 1492 | 4401 | 3479 | 4638 | 4518 | 10.2% |
| d4 | 4 | 4 | 5991 | 2749 | 1523 | 30 | 421 | 404 | 284 | 430 | 7.2% |
| ref | 4 | 3 | 43207 | 9967 | 18345 | 25 | 6002 | 2257 | 22 | 6588 | 15.2% |
...
- candidats AVEC connectivite : 4645, dont T2 : 100%
- candidats SANS connectivite : 6898, dont T2 : 100%
- **l'hypothese ne tient pas — le v2 n'est qu'un v1 elargi**

### censure de l'echantillon

- **4912 systemes avec CONNECTED sur 38544 (12.7%) sont abandonnes en TROP-CHER** et ne figurent donc pas dans la mesure ci-dessus.
...
*7253 candidats sur 11543 portent les champs bruts (63%).*
...
- `ab89a63b01ef` — 2280 candidats
  ...
  - test de permutation : **p = 0.0005** — significatif.
- `5c556c16ea8b` — 1288 candidats
  - AVEC connectivite (517) : resistance **41.2%**
  - SANS connectivite (771) : resistance **22.0%**
  - test de permutation : **p = 0.0005** — significatif.
...
- `ab89a63b01ef` — 2280 candidats
  ...
  - test de permutation : **p = 0.0005** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `5c556c16ea8b` — 1288 candidats
  - AVEC connectivite (517) : T0=13.02 T1=0.00 T2=3.00 — pondere **6.00**
  - SANS connectivite (771) : T0=15.76 T1=0.00 T2=2.66 — pondere **5.32**
  - test de permutation : **p = 0.0020** — ecart significatif au seuil 0.05, sur une serie reproductible.
- `06fe04a859f1` — 1195 candidats
...
**7 serie(s) reproductible(s) sur 14 etablissent l'ecart** : `89c65c03c4ad` (p=0.0005), `ab89a63b01ef` (p=0.0005), `5c556c16ea8b` (p=0.0020), `06fe04a859f1` (p=0.0005), `e3baecf8377b` (p=0.0005), `23303c299f39` (p=0.0060), `b71bb0907fb5` (p=0.0110).
...
- `T2` indices=0.06 — PAIRDIFF(>=1)@knight + CONNECTED(v0) + COUNT(v0,1-2)@grid + PAIRSTEP(1)@knight
- `T2` indices=0.06 — PAIRSTEP(1)@knight + PAIRDIFF(>=1)@knight + CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,1-4)@grid
- `T2` indices=0.06 — PAIRDIFF(>=1)@adj + CONNECTED(v2) + COUNT(v2,1-2)@grid + PAIRSTEP(1)@knight
- `T2` indices=0.10 — MONO@cols + SUM(1+-1)@rows
- `T2` indices=0.12 — SUM(1+-1)@cols + MONO@rows
- `T2` indices=0.12 — SUM(1+-1)@rows + MONO@cols
- `T2` indices=0.12 — SUM(7+-1)@cols + MONO@blocks + MONO@rows
- `T2` indices=0.12 — PAIRDIFF(>=1)@adj + PAIRDIFF(>=1)@knight + CONNECTED(v0) + COUNT(v0,1-4)@grid
- `T2` indices=0.12 — PAIRSTEP(1)@adj + CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,4-5)@grid + PAIRDIFF(>=1)@knight
- `T2` indices=0.12 — PAIRDIFF(>=1)@adj + CONNECTED(v2) + COUNT(v2,1-5)@grid + PAIRDIFF(>=1)@knight
- `T2` indices=0.12 — PAIRDIFF(>=1)@knight + CONNECTED(v1) + NOSQUARE(v1) + COUNT(v1,1-5)@grid + PAIRDIFF(>=1)@adj
- `T2` indices=0.12 — CONNECTED(v2) + NOSQUARE(v2) + COUNT(v2,1-5)@grid + PAIRDIFF(>=1)@adj + PAIRDIFF(>=1)@adj
...
## cout
- temps total 28.7 h, dont 2% brule sur des systemes MORT
- TROP-CHER : 4944 systemes abandonnes (5.3% des systemes), 96% du temps total
  dont 4912 avec CONNECTED, 32 sans -- **chiffre CONFONDU** : seul le tag connect peut produire des systemes avec CONNECTED, ce ratio melange l'effet de la connectivite et celui de la configuration. Voir la ventilation ci-dessous.
- taux de TROP-CHER **dans le seul tag connect** (a configuration egale, non confondu) :
  - avec CONNECTED : 12.9% sur 36043 systemes
  - sans CONNECTED : 0.0% sur 8068 systemes
